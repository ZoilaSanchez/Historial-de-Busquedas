...
ruta = getwd()
rutaGuardar = paste(ruta, "AGraficar/enlacesmasbuscados.txt", sep="/")
sink(rutaGuardar)
mayores=subset(filtrardatosunicos,(as.numeric(canturl))>=300)
for(i in 1:length(mayores$listurl)){

  
 # print(mayores$listurl[i][1])
  if(mayores$listurl[i][1]!=""){
    cat(mayores$canturl[i][1])
    cat("\n")
    cat(mayores$listurl[i][1])
    cat("\n")
...
listaLinks<- titulo
#----------------------------------------------------------------------------------
# Código para hallar los meses y dias de una página
#----------------------------------------------------------------------------------
